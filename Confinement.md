...
\begin{equation}
    \frac {3} {2} \left( \frac {P} {f_{\rm th} \rho} \right)
        = \left( \frac {3 f_\varphi} {2 \alpha_{\rm eff}} \right) v_{\rm c}^2 (r)
\end{equation}
when a combination of thermal and turbulent energy supports a steady-state galactic atmosphere. (See  \textbf{\textsc{ExpCGM:} The Essentials} for an explanation and and definitions of these symbols.) If there are other forms of support energy, then the leading numerical coefficient (3/2) may be different. The specific gravitational binding energy of the gas at radius $r$ is
\begin{equation}
    \epsilon_{\rm bind} (r) = \left( \frac {3 f_\varphi} 
                               {2 \alpha_{\rm eff}} \right) v_{\rm c}^2 (r)
...
\begin{equation}
    \alpha_{\rm eff}(r) > \frac {3 f_\varphi} {2} 
                          \frac {v_{\rm c}^2(r)} {\varphi_\infty - \varphi(r)} 
        \; \; .
\end{equation}
...
\begin{equation}
    v_{\rm c}^2 (x) 
        = A_{\rm NFW} v_\varphi^2 \left[ \frac {\ln (1 + x)} {x} - \frac {1} {1 + x} \right]
        \; \; ,
\end{equation}
...
\begin{equation}
    \alpha_{\rm eff} \: > \: 
       \frac {3} {2} \left[ 1 - \frac {x} { (1 + x) \ln (1 + x) } \right]
    \; \; .
\end{equation}
Notice that $\alpha_{\rm eff} = 3/2$ is a critical value for gravitational confinement: 
\begin{itemize}
    \item{All layers having $\alpha_{\rm eff} > 3/2$ are gravitationally bound, because each layer's specific support energy $3 v_{\rm c}^2(r) / 2 \alpha_{\rm eff} (r)$ is less than the specific energy $v_{\rm c}^2(r) = G M_r / r$ required to escape the gravitational attraction of the total mass $M_r$ within $r$. (This general result does not depend on the details of $\varphi$.)} 
    \item{Gravitational confinement at small radii ($r \ll r_{\rm s}$) does not depend on $\alpha_{\rm eff}$ because $v_{\rm c}^2 \ll \varphi_\infty$. (This result applies to potential wells in which $v_c^2$ goes to zero as $r \rightarrow 0$, as in the NFW potential.)} 
    \item{Atmospheric layers that have $\alpha_{\rm eff} < 3/2$ can be pressure confined by the weight of overlying layers, but the pressure profile must steepen to $\alpha_{\rm eff} > 3/2$ at larger radii in order for the entire atmosphere to be gravitationally confined.}
    \item{An atmosphere with $\alpha_{\rm eff} < 3/2$ near its outer boundary needs to be confined by external pressure forces.}
\end{itemize}
 If the radial gradient of the thermal support fraction $f_{\rm th}$ is insignificant, then $\alpha$ replaces $\alpha_{\rm eff}$ in this bullet list. The atmospheres of galaxy clusters appear to abide by the constraints listed above, since observations of their thermal pressure profiles\footnote{For example, see Pointecouteau et al. (2021, A\&A, 651, A73) } show that $\alpha \lesssim 1$ at $r \ll r_{\rm s}$ and $\alpha > 2$ at $r \gg r_{\rm s}$. 

In the \textsc{ExpCGM} framework, raising an atmosphere's specific energy to $E_{\rm CGM} / M_{\rm CGM} > \varphi_\infty$ restricts the set of pressure profiles that can be in a static equilibrium configuration. For example, the maximum value of mean specific energy in an atmosphere with $\alpha_{\rm eff} > 3/2$ cannot exceed $\varphi_\infty$, which is $4.625 v_\varphi^2$ in an NFW potential. Energy input comparable to $M_{\rm CGM} v_\varphi^2$ can exponentially expand the atmosphere, but energy input that raises $E_{\rm CGM}/M_{\rm CGM}$ to $\varphi_\infty$ drives the atmosphere's radius to infinity and its pressure normalization to zero. In that limit, the assumption of gravitational confinement for $\alpha_{\rm eff} > 3/2$ is somewhat artificial but is still useful for quantifying the connection between an atmosphere's specific energy $E_{\rm CGM} / M_{\rm CGM}$ and its pressure normalization factor $P_0$.


\section{Evolving Potential}
\label{sec:EvolvingPotential}
...
\section{Extended Potential}

To account for atmospheric confinement by matter extending beyond the halo, the \textsc{ExpCGM} framework includes an extended potential well that is unbounded. Numerical simulations of cosmological structure formation\footnote{For example, see Diemer \& Kravtsov (2014, ApJ, 789, 1)} indicate that the total mass density $\rho_M$ at radii several times $R_{\rm halo}$ declines with an approximate power-law dependence on radius ranging from $r^{-1}$ to $r^{-1.5}$. The framework therefore accounts for the gravitational potential of matter external to the halo using the extended potential
\begin{equation}
    \varphi_{\rm ext} (r) 
        = 2 \pi G \rho_{\rm ext} R_{\rm halo}^2 
            \left( \frac {r} {R_{\rm halo}} - 1 \right) \left( 1 - \frac {R_{\rm halo}} {r} \right) 
\end{equation}
...
\begin{equation}
    \varphi_{\rm ext} (r) 
        = \frac {G \dot{M}_{\rm halo}} {v_{\rm acc}} 
            \left( \frac {r} {R_{\rm halo}} - 1 \right) \left( 1 - \frac {R_{\rm halo}} {r} \right) 
\end{equation}
...
\begin{equation}
    \frac {G \dot{M}_{\rm halo}} {v_{\rm acc}} 
        = \left( \frac {G M_{\rm halo}} {R_{\rm halo}} \right) 
            \left( \frac {R_{\rm halo}} {v_{\rm acc}} 
                \frac {\dot{M}_{\rm halo}} {M_{\rm halo}} \right)
    \; \; .
\end{equation}
...
In the \textsc{ExpCGM} framework, $f_{\rm acc}$ is a model parameter that specifies the normalization of the extended potential:
\begin{equation}
    \varphi_{\rm ext} (r) 
        = f_{\rm acc} \left( \frac {G M_{\rm halo}} {R_{\rm halo}} \right)
           \left( \frac {r} {R_{\rm halo}} - 1 \right) \left( 1 - \frac {R_{\rm halo}} {r} \right) 
           \; \; .
\end{equation}
...
\begin{equation}
    f_{\rm acc} \approx 2 \left( \frac {\rho_{\rm cr}} {\rho_{\rm halo}} \right)^{1/2}
            \frac {\dot{M}_{\rm halo}} {H M_{\rm halo}}
            \; \; ,
\end{equation}
...
\begin{figure}[t]
\begin{center}
  \includegraphics[width=0.95\textwidth]{xCGM_ext_plot.pdf}
\end{center}
